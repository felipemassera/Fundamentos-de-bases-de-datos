...
procedure eliminarTipo(var a:archivo);
    procedure eliminarDino(var a:archivo; tipo:string );
    var
        cabecera, aux: recordDinos;
    begin
        reset(a);
        leer(a,cabecera);
        aux.codigo:= cabecera.codigo;
        while (aux.codigo<>valor_alto) do
            while ((aux.codigo<>valor_alto) and (aux.tipo<>tipo)) do begin
                leer(a,aux);
...
var
    ...
    tipo : string
begin
    write('Ingrese el tipo de dino que desea eliminar');
    read(tipo);
    while(tipo<>'100000')do begin
        eliminarDino(a,tipo);
        write('Ingrese el tipo de dino que desea eliminar');
        read(tipo);
    end;
